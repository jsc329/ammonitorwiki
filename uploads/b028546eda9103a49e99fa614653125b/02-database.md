...
tables that are related in some way. Specific columns in one table may
be formally linked to other tables. Information stored in or across
tables can be retrieved using a language called SQL, which stands for
...
[Figure
2.1.]('https://code.usgs.gov/vtcfwru/ammonitor/tree/master/vignettes/Chap2_Figs/directories.PNG')

*Figure 2.1. The “database” directory should store the AMMonitor SQLite
database.*
...
This is our first look at SQLite syntax, passed through `dbExecute()`‘s
’statement’ argument. The word PRAGMA (from “pragmatic”) is a computer
programming directive that tells SQLite how to process input. Here, we
are telling SQLite to enforce foreign key contraints. That means if a
person named Gandalf deploys equipment, and we create a new record in
the **deployment** table indicating so, Gandalf must be present in the
**people** table, or an error will occur. This ensures the database
...
`dbReadTable()` to read the entire table into R’s memory, again
...
*Figure 2.14*. Here, you can see that the AMMonitor front end comes with
many individual forms that can be customized; these individual forms are
...
*Figure 2.15*. The Navigation Form can be set as the default form that
appears when the Access database is first opened. This can be achieved
by going to File | Options | Current Database, and setting the
Navigation Form as the Display Form. You may also add a customized title
and icon here as well.
