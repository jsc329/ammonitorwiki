...
# render("Chapter 01-Getting Started with AMMonitor.Rmd")
```
...
The package itself contains many functions and built-in datasets, which we will introduce over the course of this book. Below, we display the first 10 functions and datasets as a tibble [@tibble]:
```{r, eval = T}
...
Each (empty) directory will store specific types of information as introduced below:
...
- **scripts**. Stores R scripts that can be sourced each day to automatically process new data (chapter 20).
